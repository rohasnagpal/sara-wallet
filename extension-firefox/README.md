...
- Create or import an EVM wallet — one address works across five networks
...
2. **Test a send with a small amount first**, ideally on a cheap network
   (Polygon PoS) before larger amounts or expensive chains.
3. The passphrase has no recovery — same as the main app, losing it makes
